...
func CreateCategoryOffer(c *gin.Context) {
	...
	if err != nil {
		c.JSON(http.StatusBadRequest,
			response.ClientResponse(
				http.StatusBadRequest,
				"failed to create category offer",
				nil,
				err.Error(),
			))
		return
	...
	c.JSON(http.StatusOK,
		response.ClientResponse(
			http.StatusOK,
			"category offer created successfully",
			nil,
			nil,
		))
}
...
func GetAllCategoryOffers(c *gin.Context) {
	...
	if err != nil {
		...
		errRes := response.ClientResponse(
			http.StatusBadRequest,
			"failed to fetch category offers",
			nil,
			err.Error(),
		)
	...
	successRes := response.ClientResponse(
		http.StatusOK,
		"category offers fetched successfully",
		offers,
		nil,
	)
...
func GetReferralDetails(c *gin.Context) {
	...
	if err != nil {
		c.JSON(http.StatusBadRequest,
			response.ClientResponse(
				http.StatusBadRequest,
				"failed to get referral details",
				nil,
				err.Error(),
			))
		return
